...
void VkImage_Impl::dump() {
  if (get_format_bpp(this->format) == 4) {
    write_image_2d_i32_ppm("tmp.ppm", this->get_ptr(), this->extent.width * 4, this->extent.width,
                           this->extent.height);
  } else if (get_format_bpp(this->format) == 3) {
    write_image_2d_i24_ppm("tmp.ppm", this->get_ptr(), this->extent.width * 3, this->extent.width,
                           this->extent.height);
  } else {
    UNIMPLEMENTED;
  }
...
void GPU_State::execute_commands(VkCommandBuffer_Impl *cmd_buf) {
  ...
  while (cmd_buf->has_items()) {
    ...
    case Cmd_t::BindDescriptorSets: {
      ...
      ito(cmd.descriptorSetCount) { descriptor_sets[i + cmd.firstSet] = cmd.pDescriptorSets[i]; }
      break;
    }
    case Cmd_t::BindPipeline: {
      cmd::BindPipeline cmd = cmd_buf->consume<cmd::BindPipeline>();
      if (cmd.pipelineBindPoint == VkPipelineBindPoint::VK_PIPELINE_BIND_POINT_GRAPHICS) {
    ...
    case Cmd_t::CopyBuffer: {
      ...
      ito(cmd.regionCount) {
        memcpy(cmd.dst->get_ptr() + cmd.pRegions[i].dstOffset,
               cmd.src->get_ptr() + cmd.pRegions[i].srcOffset, cmd.pRegions[i].size);
      }
      break;
    }
    ...
    case Cmd_t::DrawIndexed: {
      ...
      draw_indexed(this, cmd.indexCount, cmd.instanceCount, cmd.firstIndex, cmd.vertexOffset,
                   cmd.firstInstance);
      break;
    }
    case Cmd_t::CopyBufferToImage: {
      ...
      ito(cmd.regionCount) {
        ...
        // tightly packed
        if (buffer_pitch == 0)
          buffer_pitch = cmd.pRegions[i].imageExtent.width * get_format_bpp(cmd.dstImage->format);
        size_t buffer_height = cmd.pRegions[i].bufferImageHeight;
        if (buffer_height == 0) buffer_height = cmd.pRegions[i].imageExtent.height;
        size_t bytes_per_layer = buffer_pitch * buffer_height;
        uto(cmd.pRegions[i].imageSubresource.layerCount) {
          kto(buffer_height) {
            memcpy(cmd.dstImage->get_ptr(cmd.pRegions[i].imageSubresource.mipLevel,
                                         cmd.pRegions[i].imageSubresource.baseArrayLayer + u),
                   cmd.srcBuffer->get_ptr() + cmd.pRegions[i].bufferOffset + bytes_per_layer * u,
...
void *allocate_trap(char const *fun_name) {
  ...
  if (executable_memory == NULL) {
    ...
    executable_memory =
        (uint8_t *)mmap(NULL, allocated_memory_size, PROT_READ | PROT_WRITE | PROT_EXEC,
...
VKAPI_ATTR void VKAPI_CALL vkGetPhysicalDeviceProperties(VkPhysicalDevice            physicalDevice,
  ...
  pProperties->deviceType    = VkPhysicalDeviceType::VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU;
...
VKAPI_ATTR void VKAPI_CALL vkGetPhysicalDeviceQueueFamilyProperties(
  ...
  *pQueueFamilyPropertyCount           = 1;
  ...
  pQueueFamilyProperties[0].queueFlags = VkQueueFlagBits::VK_QUEUE_COMPUTE_BIT |
...
VKAPI_ATTR void VKAPI_CALL vkGetPhysicalDeviceFormatProperties(
  ...
  pFormatProperties->bufferFeatures = VkFormatFeatureFlagBits::VK_FORMAT_FEATURE_FLAG_BITS_MAX_ENUM;
...
VKAPI_ATTR void VKAPI_CALL vkGetPhysicalDeviceMemoryProperties(
  ...
  pMemoryProperties->memoryHeaps[0].flags = VkMemoryHeapFlagBits::VK_MEMORY_HEAP_DEVICE_LOCAL_BIT;
...
VKAPI_ATTR VkResult VKAPI_CALL vkQueueSubmit(VkQueue queue, uint32_t submitCount,
  ...
  ito(submitCount) {
    jto(pSubmits[i].commandBufferCount) {
      vki::VkCommandBuffer_Impl *impl = (vki::VkCommandBuffer_Impl *)pSubmits[i].pCommandBuffers[j];
...
VKAPI_ATTR void VKAPI_CALL vkUnmapMemory(VkDevice device, VkDeviceMemory memory) { return; }
...
VKAPI_ATTR VkResult VKAPI_CALL vkGetQueryPoolResults(VkDevice device, VkQueryPool queryPool,
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateImage(VkDevice device, const VkImageCreateInfo *pCreateInfo,
  ...
  ito(img->mipLevels) {
    ...
    total_mem_size += vki::get_mip_size(img->extent.width, i) *
                      ...
                      vki::get_mip_size(img->extent.depth, i) * vki::get_format_bpp(img->format);
  }
...
VKAPI_ATTR VkResult VKAPI_CALL vkMergePipelineCaches(VkDevice device, VkPipelineCache dstCache,
...
vkCreateGraphicsPipelines(VkDevice device, VkPipelineCache pipelineCache, uint32_t createInfoCount,
  ...
  ito(createInfoCount) {
    ...
    impl->RS_state.depthClampEnable        = info.pRasterizationState->depthClampEnable;
    impl->RS_state.rasterizerDiscardEnable = info.pRasterizationState->rasterizerDiscardEnable;
    ...
    impl->RS_state.depthBiasConstantFactor = info.pRasterizationState->depthBiasConstantFactor;
    impl->RS_state.depthBiasClamp          = info.pRasterizationState->depthBiasClamp;
    impl->RS_state.depthBiasSlopeFactor    = info.pRasterizationState->depthBiasSlopeFactor;
    ...
    impl->DS_state.depthBoundsTestEnable   = info.pDepthStencilState->depthBoundsTestEnable;
    impl->DS_state.stencilTestEnable       = info.pDepthStencilState->stencilTestEnable;
    ...
    impl->OM_blend_state.attachmentCount   = info.pColorBlendState->attachmentCount;
    jto(impl->OM_blend_state.attachmentCount) impl->OM_blend_state.pAttachments[j] =
        info.pColorBlendState->pAttachments[j];
    jto(4) impl->OM_blend_state.blendConstants[j] = info.pColorBlendState->blendConstants[j];
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreatePipelineLayout(VkDevice                          device,
  ...
  ito(impl->setLayoutCount) {
    impl->pSetLayouts[i] = (vki::VkDescriptorSetLayout_Impl *)pCreateInfo->pSetLayouts[i];
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateDescriptorSetLayout(
  ...
  impl->pBindings = (VkDescriptorSetLayoutBinding *)malloc(sizeof(VkDescriptorSetLayoutBinding) *
                                                           impl->bindingCount);
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateDescriptorPool(VkDevice                          device,
  ...
  impl->pPoolSizes =
      (VkDescriptorPoolSize *)malloc(sizeof(VkDescriptorPoolSize) * impl->poolSizeCount);
...
VKAPI_ATTR VkResult VKAPI_CALL
vkAllocateDescriptorSets(VkDevice device, const VkDescriptorSetAllocateInfo *pAllocateInfo,
  ...
  ito(pAllocateInfo->descriptorSetCount) {
    ...
    impl->pool->refcnt++;
    impl->layout = (vki::VkDescriptorSetLayout_Impl *)pAllocateInfo->pSetLayouts[i];
    impl->layout->refcnt++;
    ...
    memset(impl->slots, 0, sizeof(vki::VkDescriptorSet_Impl::Slot) * impl->layout->bindingCount);
...
VKAPI_ATTR VkResult VKAPI_CALL vkFreeDescriptorSets(VkDevice               device,
  ...
  ito(descriptorSetCount) { RELEASE_VKOBJ((VkDescriptorSet)pDescriptorSets[i], VkDescriptorSet); }
  return VK_SUCCESS;
}
...
VKAPI_ATTR void VKAPI_CALL vkUpdateDescriptorSets(VkDevice device, uint32_t descriptorWriteCount,
  ...
  ito(descriptorWriteCount) {
    ...
    if (write.pImageInfo != NULL) {
      ...
      impl->slots[write.dstBinding].sampler = (vki::VkSampler_Impl *)write.pImageInfo->sampler;
    ...
    if (write.pBufferInfo != NULL) {
      ...
      impl->slots[write.dstBinding].buffer = (vki::VkBuffer_Impl *)write.pBufferInfo->buffer;
    ...
    if (write.pTexelBufferView != NULL) {
      ...
      impl->slots[write.dstBinding].buf_view = (vki::VkBufferView_Impl *)write.pTexelBufferView;
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateFramebuffer(VkDevice                       device,
  ...
  ito(impl->attachmentCount) {
    impl->pAttachments[i] = (vki::VkImageView_Impl *)pCreateInfo->pAttachments[i];
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateRenderPass(VkDevice                      device,
  ...
  ito(impl->subpassCount) {
    vki::VkRenderPass_Impl::VkSubpassDescription_Impl *simpl = &impl->pSubpasses[i];
    simpl->flags                                             = pCreateInfo->pSubpasses[i].flags;
    simpl->colorAttachmentCount    = pCreateInfo->pSubpasses[i].colorAttachmentCount;
    simpl->inputAttachmentCount    = pCreateInfo->pSubpasses[i].inputAttachmentCount;
    simpl->pipelineBindPoint       = pCreateInfo->pSubpasses[i].pipelineBindPoint;
    simpl->preserveAttachmentCount = pCreateInfo->pSubpasses[i].preserveAttachmentCount;
    ...
    if (pCreateInfo->pSubpasses[i].pDepthStencilAttachment != NULL) {
      ...
      simpl->pDepthStencilAttachment      = pCreateInfo->pSubpasses[i].pDepthStencilAttachment[0];
    }
  }
...
VKAPI_ATTR void VKAPI_CALL vkFreeCommandBuffers(VkDevice device, VkCommandPool commandPool,
  ...
  ito(commandBufferCount) {
    vki::VkCommandBuffer_Impl *impl = (vki::VkCommandBuffer_Impl *)pCommandBuffers[i];
    impl->release();
  }
  return;
}
...
VKAPI_ATTR void VKAPI_CALL vkCmdBindDescriptorSets(
  ...
  ito(descriptorSetCount) cmd.pDescriptorSets[i] = (vki::VkDescriptorSet_Impl *)pDescriptorSets[i];
...
VKAPI_ATTR void VKAPI_CALL vkCmdDrawIndirect(VkCommandBuffer commandBuffer, VkBuffer buffer,
...
VKAPI_ATTR void VKAPI_CALL vkCmdDrawIndexedIndirect(VkCommandBuffer commandBuffer, VkBuffer buffer,
                                                    VkDeviceSize offset, uint32_t drawCount,
                                                    uint32_t stride) {
  vki::VkCommandBuffer_Impl *   impl = (vki::VkCommandBuffer_Impl *)commandBuffer;
...
VKAPI_ATTR void VKAPI_CALL vkCmdDispatch(VkCommandBuffer commandBuffer, uint32_t groupCountX,
...
VKAPI_ATTR void VKAPI_CALL vkCmdDispatchIndirect(VkCommandBuffer commandBuffer, VkBuffer buffer,
                                                 VkDeviceSize offset) {
  vki::VkCommandBuffer_Impl *impl = (vki::VkCommandBuffer_Impl *)commandBuffer;
...
VKAPI_ATTR void VKAPI_CALL vkCmdCopyImage(VkCommandBuffer commandBuffer, VkImage srcImage,
...
VKAPI_ATTR void VKAPI_CALL vkCmdBlitImage(VkCommandBuffer commandBuffer, VkImage srcImage,
                                          VkImageLayout srcImageLayout, VkImage dstImage,
                                          VkImageLayout dstImageLayout, uint32_t regionCount,
...
VKAPI_ATTR void VKAPI_CALL vkCmdUpdateBuffer(VkCommandBuffer commandBuffer, VkBuffer dstBuffer,
...
VKAPI_ATTR void VKAPI_CALL vkCmdFillBuffer(VkCommandBuffer commandBuffer, VkBuffer dstBuffer,
                                           VkDeviceSize dstOffset, VkDeviceSize size,
                                           uint32_t data) {
  return;
}

VKAPI_ATTR void VKAPI_CALL vkCmdClearColorImage(VkCommandBuffer commandBuffer, VkImage image,
...
VKAPI_ATTR void VKAPI_CALL vkCmdSetEvent(VkCommandBuffer commandBuffer, VkEvent event,
...
VKAPI_ATTR void VKAPI_CALL vkCmdBeginQuery(VkCommandBuffer commandBuffer, VkQueryPool queryPool,
                                           uint32_t query, VkQueryControlFlags flags) {
  return;
}

VKAPI_ATTR void VKAPI_CALL vkCmdEndQuery(VkCommandBuffer commandBuffer, VkQueryPool queryPool,
                                         uint32_t query) {
  return;
}
...
VKAPI_ATTR void VKAPI_CALL vkCmdBeginRenderPass(VkCommandBuffer              commandBuffer,
  ...
  ito(cmd.clearValueCount) cmd.pClearValues[i] = pRenderPassBegin->pClearValues[i];
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateSwapchainKHR(VkDevice                        device,
  ...
  NOTNULL(pSwapchain);
  vki::VkSurfaceKHR_Impl *surface = (vki::VkSurfaceKHR_Impl *)pCreateInfo->surface;
...
VKAPI_ATTR VkResult VKAPI_CALL vkQueuePresentKHR(VkQueue                 queue,
  ...
  kto(pPresentInfo->swapchainCount) {
    vki::VkSwapChain_Impl * impl    = (vki::VkSwapChain_Impl *)pPresentInfo->pSwapchains[k];
    ...
    image =
        xcb_image_create_native(c, w, h, XCB_IMAGE_FORMAT_Z_PIXMAP, depth, data, w * h * 4, data);
