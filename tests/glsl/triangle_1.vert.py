...
void test_launch(void *_printf) {
  ...
  struct gl_PerVertex_t {
    float4 pos;             // offset: 0
    //float point_size;       // offset: 16
    //float clip_distance;    // offset: 20
  ...
  for (uint32_t i = 0; i < NUM_INVOCATIONS; i++) {
    ...
    TEST_EQ(tmp.x, gl_PerVertex[i].pos.x);
    TEST_EQ(tmp.y, gl_PerVertex[i].pos.y);
    TEST_EQ(tmp.z, gl_PerVertex[i].pos.z);
    TEST_EQ(tmp.w, gl_PerVertex[i].pos.w);
  }
